...
pub struct Config {
    ...
    pub sort: bool,
    /// Whether to ignore case when matching. Note that when this is set to true, and the
    /// case matches on the needle and haystack, it will receive the `scoring.match_case_bonus`.
    /// When set to false, all matches will receive the `scoring.match_case_bonus`.
